...
def generate_launch_description():
    ...
    convert_urdf_to_sdf()  # Convert before launching
    ...
    return launch.LaunchDescription([
        ...
        launch.actions.TimerAction(
            period=2.0,  # Wait before loading controllers
            actions=[
                launch_ros.actions.Node(
                    package="ros2cli",
                    executable="ros2",
                    arguments=["control", "load_controller", "--set-state", "active", "joint_state_broadcaster"],
                    output="screen"
                ),
                launch_ros.actions.Node(
                    package="ros2cli",
                    executable="ros2",
                    arguments=["control", "load_controller", "--set-state", "active", "position_controller"],
                    output="screen"
                )
            ]
        )
