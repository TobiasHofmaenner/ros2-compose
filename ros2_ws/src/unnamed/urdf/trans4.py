...
"""
            ...
            # Add gazebo_ros2_control plugin inside the joint
            gazebo_control_block = f"""
    <gazebo>
      <plugin filename="libgazebo_ros2_control.so" name="gazebo_ros2_control"/>
    </gazebo>
"""

            # Insert ros2_control block + gazebo block before `</joint>`
            modified_joint_tag = full_joint_tag.replace("</joint>", ros2_control_block + gazebo_control_block + "\n  </joint>")
            updated_content = updated_content.replace(full_joint_tag, modified_joint_tag)

            controller_joints.append(joint_name)
            ros2_control_joints.append(joint_name)

    # Ensure <ros2_control> is correctly inserted inside <robot>
    ros2_control_main_block = f"""
  <ros2_control name="{package_name}_control" type="system">
    <hardware>
      <plugin>gazebo_ros2_control/GazeboSystem</plugin>
    </hardware>
    {''.join(f"""
    ...
    """ for joint_name in ros2_control_joints)}
  </ros2_control>
"""

    # Ensure the robot tag remains correctly structured
    updated_content = updated_content.replace(
        '<robot name="unnamed" version="1.0">',
        f'<robot name="unnamed" version="1.0">\n{ros2_control_main_block}'
    )
